...
public class ProfileSettingsPictureListActivity extends ComponentListActivity<BaseListItem> implements RequestControllerObserver,
		...
		if (data != null && data.getData() != null && !data.getData().toString().trim().equals("")) {
			getHandler().post(new Runnable() {
				public void run() {
                    ...
                    final String orientation = ImageHelper.getExifOrientation(localImageUri, getContentResolver(), getApplicationContext());
                    final Bitmap bitmap = ImageHelper.createThumbnail(localImageUri, getContentResolver(), IMAGE_SIZE, orientation);
	...
	private void startSubmitPicture(Bitmap bitmap, Uri localImageUri) {
		...
		final boolean success = LocalImageStorage.putBitmap(this, imageUrl, bitmap);
		if (success) {
