...
class StandardScoreloopManager implements ScoreloopManager, Manager, Factory, ShortcutObserver, Delegate, ValueSourceFactory,
	...
	public ScreenDescription createAchievementScreenDescription(final User user) {
		...
		description.setBodyDescription(new Intent(getContext(), AchievementListActivity.class)).getArguments().putValue(
				Constant.ACHIEVEMENTS_ENGINE, getAchievementsEngine());
		return description;
	}
	...
	public ScreenDescription createScoreScreenDescription(final Game game, final Integer mode, final Integer leaderboard) {
		...
		description.setHeaderDescription(new Intent(getContext(), ScoreHeaderActivity.class)).getArguments().putValue(
				Constant.IS_LOCAL_LEADEARBOARD, isLocalLeaderboard);

		if (isLocalLeaderboard) {
			description.setBodyDescription(new Intent(getContext(), ScoreListActivity.class)).getArguments().putValue(Constant.SEARCH_LIST,
					SearchList.getLocalScoreSearchList());
		} else {
	...
	private List<Score> getLocalScoresToSubmit() {
		...
		final ScoresController scoresController = new ScoresController(getSession(), this);
	...
	public void onGamePlayEnded(final Score score, final Boolean submitLocallyOnly) {
		...
		if (_challengeOngoing) {
			...
			_lastChallenge = null;
			getScoreController().setShouldSubmitScoreLocally((submitLocallyOnly != null) && submitLocallyOnly);
	...
	public void requestControllerDidFail(final RequestController aRequestController, final Exception anException) {
		if ((aRequestController == _scoreController) || (aRequestController == _challengeController)) {
			...
			if (anException instanceof ScoreSubmitException) {
				_lastStatus = OnScoreSubmitObserver.STATUS_SUCCESS_LOCAL_SCORE;
			} else {
	...
	public void requestControllerDidReceiveResponse(final RequestController aRequestController) {
		if ((aRequestController == _scoreController) || (aRequestController == _challengeController)) {
			if (_challengeOngoing) {
				...
				_challengeOngoing = false;
			} else if (getScoreController().shouldSubmitScoreLocally()) {
				_lastStatus = OnScoreSubmitObserver.STATUS_SUCCESS_LOCAL_SCORE;
			} else {
