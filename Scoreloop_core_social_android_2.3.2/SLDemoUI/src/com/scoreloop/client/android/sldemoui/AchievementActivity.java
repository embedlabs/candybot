...
public class AchievementActivity extends Activity {
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		_progressButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(final View v) {
				// to bring up a toast (2nd argument) and sync it right away (3rd argument):
				ScoreloopManagerSingleton.get().achieveAward(_achievement.getAward().getIdentifier(), true, true);

				// alternatively set achievement to achieved and call syncAchievements() at some later point
	...
	private void updateUI() {
		...
		if (_achievement.isAchieved()) {
			progressText.setText("Already unlocked");
			_progressButton.setEnabled(false);
		} else {
			progressText.setText("Not yet unlocked");
		}
	}
}
