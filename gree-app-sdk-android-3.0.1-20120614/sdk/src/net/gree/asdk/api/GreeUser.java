 ...
 */
 ...
 /**
  * This will load the user thumbnail for the given size, this is an asynchronous call, non-blocking,
  * the listener will be called back when the request is done.
  * pass in the {@link IconDownloadListener} to get the result of the request.
  * If the image was already requested 
  * 
  * @param size one of DEFAULT,
  ...
  */
  public boolean loadThumbnail(int size, final IconDownloadListener listener) {
    BitmapLoader loader = null;
    switch (size) {
      case THUMBNAIL_SIZE_SMALL:
        if (mThumbnailSmallLoader == null) {
          mThumbnailSmallLoader = BitmapLoader.newLoader(getId(), thumbnailUrlSmall, new BitmapClientWrapper(new BitmapClient()));
        }
        loader = mThumbnailSmallLoader;
        break;
      case THUMBNAIL_SIZE_LARGE:
        if (mThumbnailLargeLoader == null) {
          mThumbnailLargeLoader = BitmapLoader.newLoader(getId(), thumbnailUrlLarge, new BitmapClientWrapper(new BitmapClient()));
        }
        loader = mThumbnailLargeLoader;
        break;
      case THUMBNAIL_SIZE_HUGE:
        if (mThumbnailHugeLoader == null) {
          mThumbnailHugeLoader = BitmapLoader.newLoader(getId(), thumbnailUrlHuge, new BitmapClientWrapper(new BitmapClient()));
        }
        loader = mThumbnailHugeLoader;
        break;
      case THUMBNAIL_SIZE_STANDARD:
      default:
        if (mThumbnailLoader == null) {
          mThumbnailLoader = BitmapLoader.newLoader(getId(), thumbnailUrl, new BitmapClientWrapper(new BitmapClient()));
        }
        loader = mThumbnailLoader;

    }
    if (loader == null) {
      return false;
    }
    return tryLoadThumbnail(loader, listener);
  }
  ...
  public Bitmap getThumbnail() {
    if (mThumbnailLoader == null) {
      mThumbnailLoader =
          BitmapLoader.newLoader(getId(), thumbnailUrl, new BitmapClientWrapper(
              new BitmapClient()));
    }
  ...
  public Bitmap getSmallThumbnail() {
    if (mThumbnailSmallLoader == null) {
      mThumbnailSmallLoader =
          BitmapLoader.newLoader(getId(), thumbnailUrlSmall, new BitmapClientWrapper(
              new BitmapClient()));
    }
    if (mThumbnailSmallLoader != null) {
  ...
  public Bitmap getLargeThumbnail() {
    if (mThumbnailLargeLoader == null) {
      mThumbnailLargeLoader =
          BitmapLoader.newLoader(getId(), thumbnailUrlLarge, new BitmapClientWrapper(
              new BitmapClient()));
    }
    if (mThumbnailLargeLoader != null) {
  ...
  public Bitmap getHugeThumbnail() {
    if (mThumbnailHugeLoader == null) {
      mThumbnailHugeLoader =
          BitmapLoader.newLoader(getId(), thumbnailUrlHuge, new BitmapClientWrapper(
              new BitmapClient()));
    }
    if (mThumbnailHugeLoader != null) {
...
// an array with 1 entry
  ...
  * <p>By checking this field, you can distinguish campaign accounts such as celebrity accounts from standard users.
  ...
  * "special" means A campaign account set up for promotional use within GREE.</BR>
  * "staff" means A staff account.</BR>
  * "official" means An official account within GREE.</BR>
  ...
  */
   ...
   * upon failure call updateSelfAndLoad to refresh user and retry
   * 
   * @param thumbnailLoader 
   * @param listener
   */
  private boolean tryLoadThumbnail(final BitmapLoader thumbnailLoader, final IconDownloadListener listener) {
    //prepare Listener
    IconDownloadListener localListener = new IconDownloadListener() {
      @Override
      public void onSuccess(Bitmap image) {
        if (isVerbose) {
          GLog.v(TAG, "Try to get Thumbnail");
        }
        if (listener != null) {
      ...
      public void onFailure(int responseCode, HeaderIterator headers, String response) {
        if (isVerbose) {
          GLog.v(TAG, "Failed to get Thumbnail, try to refresh User before retry");
        }
        updateSelfAndLoad(thumbnailLoader, listener);
      }
    };

    //try loading
    return thumbnailLoader.load(localListener, false);
  }
   ...
   */
  private void updateSelfAndLoad(final BitmapLoader thumbnailLoader, final IconDownloadListener listener) {
    if (isVerbose) {
    ...
    GreeUserListener userListener = new GreeUserListener() {
      ...
      public void onSuccess(int index, int count, GreeUser[] users) {
        ...
        //update current instance with the new urls
        GreeUser.this.thumbnailUrl = users[0].thumbnailUrl;
        if (mThumbnailLoader != null) {
          mThumbnailLoader.setUrl(users[0].thumbnailUrl);
        }
        GreeUser.this.thumbnailUrlSmall = users[0].thumbnailUrlSmall;
        if (mThumbnailSmallLoader != null) {
          mThumbnailSmallLoader.setUrl(users[0].thumbnailUrl);
        }
        GreeUser.this.thumbnailUrlLarge = users[0].thumbnailUrlLarge;
        if (mThumbnailLargeLoader != null) {
          mThumbnailLargeLoader.setUrl(users[0].thumbnailUrl);
        }
        GreeUser.this.thumbnailUrlHuge = users[0].thumbnailUrlHuge;
        if (mThumbnailHugeLoader != null) {
          mThumbnailHugeLoader.setUrl(users[0].thumbnailUrl);
        }
        thumbnailLoader.load(listener, false);
      }
