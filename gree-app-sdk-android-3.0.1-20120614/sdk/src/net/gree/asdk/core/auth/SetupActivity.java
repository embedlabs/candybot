...
public class SetupActivity extends Activity {
  ...
  private Handler mHandler = new Handler() {
    ...
    public void handleMessage(Message message) {
      if (!AuthorizerCore.getInstance().hasOAuthAccessToken() && message.what == SHOULD_LOGIN && mDialog != null) {
        AuthorizerCore.getInstance().retrieveRequestToken(SetupActivity.this, new OnOAuthResponseListener<String>() {
          public void onSuccess(String response) {
            mDialog.loadUrl(response);
          }

          public void onFailure(OAuthException e) {
            OAuthUtil.handleException(e, SetupActivity.this, mDialog.mWebView, mDialog.getWebViewClient(), new OnCloseOAuthAlertListener() {
              public void onClose() {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    String setupUrl = Core.get(InternalSettings.SetupUrl);
    if (TextUtils.isEmpty(setupUrl)) {
  ...
  private void retrieveAccessToken(Uri uri) {
    ...
    if (url.startsWith(Scheme.getAccessTokenScheme())) {
      ...
      mDialog.stopLoading();
      AuthorizerCore.getInstance().retrieveAccessToken(this, url, new OnOAuthResponseListener<Void>() {
  ...
  public class SetupDialog extends WebViewPopupDialog {
    ...
    protected void reloadWebView() {
      if (needUuid(mUrl) && updateUuid(mUrl)) return;
      super.reloadWebView();
    }
    ...
    private void startAuthorization(final String url) {
      ...
      AuthorizerCore.getInstance().retrieveRequestToken(getContext(), new OnOAuthResponseListener<String>() {
        public void onSuccess(String response) {
          boolean ret = false;
          try {
            JSONArray applist = null;
            Uri uri = Uri.parse(url);
            String authorizeUrl = new String(response);
            String regCode = uri.getQueryParameter(QUERY_PARAM_REG_CODE);
