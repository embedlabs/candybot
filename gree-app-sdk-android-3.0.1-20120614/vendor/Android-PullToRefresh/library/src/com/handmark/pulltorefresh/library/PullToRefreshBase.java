...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		int[] PullToRefreshAttrs = {RR.attr("ptrAdapterViewBackground"), 
		                            RR.attr("ptrHeaderBackground"),
		                            RR.attr("ptrHeaderTextColor"),
		                            RR.attr("ptrMode")};
		// Styleables from XML
		TypedArray a = context.obtainStyledAttributes(attrs, PullToRefreshAttrs);
		if (a.hasValue(RR.styleable("PullToRefresh_ptrMode"))) {
			mMode = a.getInteger(RR.styleable("PullToRefresh_ptrMode"), MODE_PULL_DOWN_TO_REFRESH);
		}
		...
		String pullLabel = context.getString(RR.string("gree_pull_to_refresh_pull_label"));
		String refreshingLabel = context.getString(RR.string("gree_pull_to_refresh_refreshing_label"));
		String releaseLabel = context.getString(RR.string("gree_pull_to_refresh_release_label"));
		...
		// Styleables from XML
		if (a.hasValue(RR.styleable("PullToRefresh_ptrHeaderBackground"))) {
			setBackgroundResource(a.getResourceId(RR.styleable("PullToRefresh_ptrHeaderBackground"), Color.WHITE));
		}
		if (a.hasValue(RR.styleable("PullToRefresh_ptrAdapterViewBackground"))) {
			mRefreshableView.setBackgroundResource(a.getResourceId(RR.styleable("PullToRefresh_ptrAdapterViewBackground"),
					Color.WHITE));
		}
		a.recycle();
		a = null;
