		...
		try {
			...
			case Default:
			{
				...
				if (compressedLength < uncompressedData.length) {
					...
					OpenFeintInternal.log(TAG, String.format("Using Default strategy: orig %d bytes, compressed %d bytes (%.2f%% of original size)", uncompressedData.length, compressedLength, 100.f*((float)compressedLength/(float)uncompressedData.length)));
				} else {
			...
			case LegacyHeaderless:
				...
				OpenFeintInternal.log(TAG, String.format("Using Default strategy: orig %d bytes, compressed %d bytes (%.2f%% of original size)", uncompressedData.length, uploadData.length, 100.f*((float)uploadData.length/(float)uncompressedData.length)));
				break;
			default:
