	...
	@Override protected boolean createView() {
		...
		if (achievement.isUnlocked) {
			...
			if (0 == achievement.gamerscore) {
				...
				displayView.findViewById(R.id.of_achievement_score).setVisibility(View.INVISIBLE);
			}
		} else {
			displayView.findViewById(R.id.of_achievement_score_icon).setVisibility(View.INVISIBLE);
		}
		
		((TextView)displayView.findViewById(R.id.of_achievement_text)).setText(
				(achievement.title != null && achievement.title.length() > 0)
				? achievement.title
				: OpenFeintInternal.getRString(R.string.of_achievement_unlocked));
