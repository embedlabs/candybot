...
public class LogDataInputStream extends InputStream {
  ...
  LogDataInputStream(LogData logdata, int chunk_size) throws FileNotFoundException {
    ...
    String hardware_version = String.format("p(%s)/m(%s)", android.os.Build.PRODUCT, android.os.Build.MODEL);
    
    mHeader = String.format(HEADER_FORMAT, hardware_version, version_code, sdk_version, os_version, mcc).getBytes();
  ...
  private int getMCC(Context context) {
    ...
    if (!TextUtils.isEmpty(networkOperator) && networkOperator.length() > 3 && !networkOperator.equals("null")) {
