...
public class CommonsHttpOAuthProvider extends AbstractOAuthProvider {
    ...
    @Override
    protected net.gree.oauth.signpost.http.HttpResponse sendRequest(HttpRequest request) throws Exception {
        HttpResponse response = httpClient.execute((HttpUriRequest) request.unwrap());
        return new HttpResponseAdapter(response);
    }

    @Override
    protected void closeConnection(HttpRequest request, net.gree.oauth.signpost.http.HttpResponse response)
            throws Exception {
        if (response != null) {
