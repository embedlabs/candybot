/*
 * Copyright (c) 2008, 2009 Netflix, Matthias Kaeppler Licensed under the Apache License, Version
...
public class OAuth {
  ...
  /**
   * Pass this value as the callback "url" upon retrieving a request token if your application
   * cannot receive callbacks (e.g. because it's a desktop app). This will tell the service provider
   * that verification happens out-of-band, which basically means that it will generate a PIN code
   * (the OAuth verifier) and display that to your user. You must obtain this code from your user
   * and pass it to {@link OAuthProvider#retrieveAccessToken(OAuthConsumer, String)} in order to
   * complete the token handshake.
   */
   ...
   * Use OAuth percent encoding (not exactly the encoding mandated by x-www-form-urlencoded).
   */
  ...
  /**
   * Construct a Map containing a copy of the given parameters. If several parameters have the same
   * name, the Map will contain the first value, only.
   */
  ...
  /**
   * Builds an OAuth header from the given list of header fields. All parameters starting in
   * 'oauth_*' will be percent encoded.
   * 
   * <pre>
   ...
   * 
   * @param kvPairs the list of key/value pairs
   * @return a string eligible to be used as an OAuth HTTP Authorization header.
   */
  public static String prepareOAuthHeader(String... kvPairs) {
    ...
    for (int i = 0; i < kvPairs.length; i += 2) {
      ...
      String value =
          kvPairs[i].startsWith("oauth_") ? OAuth.percentEncode(kvPairs[i + 1]) : kvPairs[i + 1];
  ...
  public static HttpParameters oauthHeaderToParamsMap(String oauthHeader) {
    HttpParameters params = new HttpParameters();
    if (oauthHeader == null || !oauthHeader.startsWith("OAuth ")) { return params; }
  ...
  /**
   * Helper method to concatenate a parameter and its value to a pair that can be used in an HTTP
   * header. This method percent encodes both parts before joining them.
   * 
  ...
  public static void debugOut(String key, String value) {
    ...
    if (debugListener != null)
      if (DebugListener.debugging[0]) debugListener.log("[SIGNPOST] " + key + ": " + value);
  }

  public static interface DebugListener {
    public boolean debugging[] = new boolean[1];
  ...
  public static void setDebugListener(DebugListener cb) {
    debugListener = cb;
  }
}
