...
public class StatisticsActivity extends ListActivity {
	...
	public String[] fetchStats() {
		...
		for (int i = 1; i <= 5; i++) {
			worlds.add(SaveIO.readLines("world" + i + ".cls", this));
		}
		final DecimalFormat f = new DecimalFormat("#,###");

		// TIME PLAYED
		int seconds = statisticObtainer(worlds, TOTAL_TIME_MILLIS) / 1000;
		...
		// MOVES TAKEN
		final int moves = statisticObtainer(worlds, TOTAL_MOVES);
		stats.add(getString(R.string.stats_moves_taken) + ": " + f.format(moves));

		// NUMBER OF RESTARTS
		final int restarts = statisticObtainer(worlds, TOTAL_RESTARTS);
		...
		// STARS EARNED
		final int stars = statisticObtainer(worlds, STATUS);
		...
		// NUMBER OF DEATHS
		final int deaths = statisticObtainer(worlds, TOTAL_DEATHS);
		...
		// ENEMIES DEFEATED
		final int enemies = statisticObtainer(worlds, TOTAL_DEFEATED);
